...
class Recently_Output {
     ...
     * @var		object
     */
    private $recently_image;

    public function __construct( $recent_posts, array $options = array() ) {

        $this->recents = $recent_posts;
        $this->options = $options;
        $this->admin_options = Recently_Settings::get( 'admin_options' );

        $this->recently_image = Recently_Image::get_instance();

        if ( filter_var( $this->admin_options['tools']['markup']['thumbnail']['default'], FILTER_VALIDATE_URL ) ) {
            $this->recently_image->set_default( $this->admin_options['tools']['markup']['thumbnail']['default'] );
        }

        $this->default_thumbnail_sizes = $this->recently_image->get_image_sizes();
    ...
    private function build_output() {
        ...
        if ( $this->recents->have_posts() ) {
            ...
            $this->options = Recently_Helper::merge_array_r(
    ...
    private function render_post() {
        ...
        // Thumbnail
        $post_thumbnail = $this->get_thumbnail();
        ...
        if ( $this->options['shorten_title']['active'] ) {
            ...
            $post_title = force_balance_tags( Recently_Helper::truncate( $post_title, $length, $this->options['shorten_title']['words'] ) );
        ...
        if ( $this->options['markup']['custom_html'] ) {
            ...
            $content = $this->format_content( htmlspecialchars_decode( $this->options['markup']['post-html'], ENT_QUOTES ), $data, $this->options['rating'] ). "\n";
    ...
    private function get_permalink() {
        ...
        $trid = $translate->get_object_id( $post->ID, get_post_type( $post->ID ) );
    ...
    private function get_thumbnail() {
        ...
        if (
            ...
            && $this->recently_image->can_create_thumbnails() 
        ) {

            // Create / get thumbnail from custom field
            if ( 'custom_field' == $this->admin_options['tools']['markup']['thumbnail']['source'] ) {
                ...
                if ( '' != $thumb_url ) {
                    ...
                    // Resize CF image
                    if ( $this->admin_options['tools']['markup']['thumbnail']['resize'] ) {

                        $thumbnail = $this->recently_image->get_img(
                            $post,
                            $thumb_url,
                        ...
                        $thumbnail = $this->recently_image->render_image(
                            $thumb_url,
                            array( $this->options['thumbnail']['width'], $this->options['thumbnail']['height'] ),
                            'recently-thumbnail recently_cf',
                            $post
                        );
                ...
                if (
                    ...
                    && 'featured' == $this->admin_options['tools']['markup']['thumbnail']['source']
                ) {
                    ...
                    if ( current_theme_supports( 'post-thumbnails' ) ) {
                        ...
                        if ( has_post_thumbnail( $post->ID ) ) {
                            ...
                            $thumbnail = get_the_post_thumbnail(
                                $post->ID,
                                $size,
                                array( 'class' => 'recently-thumbnail recently_featured_stock' )
                            );
    ...
    private function get_excerpt() {
        ...
        if ( '' !== $excerpt ) {
            ...
            $excerpt = Recently_Helper::truncate( $excerpt, $this->options['post-excerpt']['length'], $this->options['post-excerpt']['words'] );
